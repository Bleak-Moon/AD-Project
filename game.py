...
class Button(QToolButton): 
    def __init__(self, text, callback): #초기화(생성자)
        ...
        self.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred) #가로 크기 = sizeHint()가 최적의 크기, 그렇지만 줄거나 늘어날 수 있으며 공간이 더 있는 경우 사용할 수 있음(최소 크기 = minimumSizeHint()) + 세로 크기 = sizeHint()가 최적의 크기, 그렇지만 줄거나 늘어날 수 있음(최소 크기 = minimumSizeHint())
...
class Game(QWidget):
    ...
    def initUI(self): #화면에 출력될 창들 만들고 위치 지정하기 - 나이, 이름, 행동(~하는 중), 표정, 버튼(밥 주기, 씻기기, 재우기, 공부시키기, 놀아주기), 수치(종합, 배부름, 청결, 피로, 스트레스)
        ...
        layoutGroups = [self.hbox0, self.hbox1, self.hbox2, self.hbox3, self.hbox4, self.hbox5, self.hbox6, self.hbox7]  #수평 상자들을 그룹으로 묶기 - 뒤에 나오는 상태 표시 코드의 반복 개선 위해 사용할 것
 ...
 #수평 상자, 텍스트, 상태 수치를 각각 그룹으로 묶기 - 뒤에 나오는 버튼 생성 코드의 반복 개선 위해 사용할 것
    ...
    def button_clicked(self): #버튼이 눌렸을 때 의도한 명령 수행하게 하는 함수 정의하기
        ...
        if key == 'g 주기': #밥 주기('g 주기') 버튼이 눌린 경우
            ...
            self.hunger_text.setText(self.action.feeding(-10))
            self.clean_text.setText(self.action.washing(-10)) #한 번 씻길 때마다 피로 수치 50씩 감소시키고 배부름 수치와 청결 수치 10씩 감소시키기
            ...
            self.hunger_text.setText(self.action.feeding(-10))
            self.clean_text.setText(self.action.washing(-10))
            self.tired_text.setText(self.action.sleeping(-10)) #한 번 공부시킬 때마다 스트레스 수치 정해진 만큼 증가시키고 배부름 수치와 청결 수치 10씩 감소, 피로 수치 10씩 증가시키기
            self.status_text.setText("공부 중~") #상태 창에 '공부 중~' 출력하기
            self.age = self.tamagotchi.ageCount(self.action.study_cnt) #공부를 5의 배수만큼(5번, 10번, 15번, ...) 할 때마다 나이를 한 살씩 증가시키기
            ...
            self.hunger_text.setText(self.action.feeding(-10))
            self.clean_text.setText(self.action.washing(-10))
            self.tired_text.setText(self.action.sleeping(-10)) #한 번 공부시킬 때마다 스트레스 수치 정해진 만큼 감소시키고 배부름 수치와 청결 수치 10씩 감소, 피로 수치 10씩 증가시키기
